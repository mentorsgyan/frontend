...
const Test2 = () => {
	...
	useEffect(() => {
	  ...
	  // Check the window size on initial load
		checkWindowSize();
		setNavigatorOpen(!state.isMdOrGreater);
					...
					<div className="dark:bg-gray-800 bg-white h-screen grid grid-cols-4">
						...
						<div className={`dark:text-white ${state.isMdOrGreater ? 'col-span-3' : 'col-span-4'} flex flex-col justify-between`}>
							<div >
								...
								{selectionRequired && (<p className="bg-red-500/30 text-center mb-5 w-fit mx-10 px-5 py-2 rounded-lg">{state.english ? 'Please select an option!' : 'कृपया एक विकल्प चुनें!'}</p>)}
							</div>
							<div className="">
